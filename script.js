...
if (!primaryNav?.children.length) {
  render('[data-nav]', locatorMenuMarkup);
}

if (
  primaryNav.children.length === 1 &&
  primaryNav.querySelector('.where-to-buy-menu')
) {
  primaryNav.insertAdjacentHTML('afterbegin', primaryNavMarkup);
}
...
const findCityGuideByPostalCode = async (value) => {
  ...
  const indexedMatch = cityOptions.find((option) => option.href === indexedHref);
  if (indexedMatch) {
...
if (cityMapLayout && cityMapCopy && cityMapFrame && cityRetailerCards.length) {
  ...
  `;
  const cityPageHref = window.location.pathname.split('/').pop() || '';
